...
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/SSB CMB Monitoring/CWQ_48(17.03.2026).pdf"
...
sample_date <- pdf_data |>
  unlist() |> 
  keep(~str_detect(.x, "Sample taken")) |>
  str_extract_all("\\d{1,2}\\s[:alpha:]{3,12}\\s\\d{4}") |>
  dmy()
...
# Copy to database
tbl |>
  mutate(sample_date = ymd("2026-03-17"), analysis_completed = ymd("2026-03-19")) |>
...
WHERE sample_date = '2026-03-17' AND monitoring_group = 'routine';
```
...
```{r extract-pdf}
# Select the pdf that you want to import
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand Daily/Oelofse G.50.pdf"
...
FROM coastal.results
WHERE monitoring_group = 'daily' AND sample_date = '2026-03-20';
```
...
```{r extract-pdf}
# Select the pdf that you want to import
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/Atlantic Daily/Infinity Environmental AD.68.pdf"
...
```{sql, connection = con}
SELECT * FROM coastal.results WHERE sample_date = '2026-03-13' AND monitoring_group = 'adhoc';
```
