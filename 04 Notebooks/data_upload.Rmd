...
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/SSB CMB Monitoring/CWQ_165(01.09.2026).pdf"
...
tbl <- tibble(number = sample_number,
  ...
  mutate(
    ...
    sample_time = str_extract(text, "\\d{1,2}[hH]\\d{2}") |> str_replace("[hH]", ":"),
...
# Copy to database
tbl |> copy_to(dest = con, name = "coastal", overwrite = TRUE)
```
...
FROM coastal)
INSERT INTO coastal.results (site_id, sample_date, sample_time, analysis_completed, sample_temp_c, censored_value, filename, lab_code, monitoring_group)
SELECT * FROM cte;
```

```{sql, connection = con}
SELECT
  site_id, sample_date, sample_time, censored_value, sample_temp_c, filename
FROM coastal.results
WHERE sample_date = '2026-09-01' AND monitoring_group = 'routine';
```
...
SELECT
  'XCN02' AS site_id,
  '2026-08-14'::date AS sample_date,
  '11:25'::time AS sample_time,
  '2026-08-16'::date AS analysis_completed,
   6.6 AS sample_temp_c,
  '13' AS censored_value,
  'CWQ_153(14.08.2026).pdf' AS filename,
  'WLAB'::coastal.lab AS lab_code,
  'routine'::coastal.monitoring_group AS monitoring_group;
```

```{sql connection = con}
SELECT * FROM coastal.results WHERE sample_date = '2026-08-14' AND monitoring_group = 'routine';
```
...
# Retrieve a list of sample dates from the results table in database
database <- tbl(con, I("coastal.results")) |>
  filter(monitoring_group == "daily", site_id %in% c("XCS34", "XCS26")) |>
  distinct(filename, sample_date) |>
  collect() |>
  arrange(desc(filename))

# List of files in folder
tibble(filename = list.files(str_c("../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand Daily/", year(now())), pattern = "*.pdf")) |>
  mutate(sample_date = map(filename, ~{
    pdf_text(str_c(str_c("../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand Daily/", year(now()), "/"), .x, sep = "")) |>
      str_split("\\n") |>
      unlist() |>
      keep(~str_detect(.x, "Sample taken")) |>
      str_extract("\\d{1,2}\\s[:alpha:]{3,12}\\s\\d{4}") |>
  ...
  unnest(sample_date) |>
  filter(!filename %in% (database |> filter(year(sample_date) == year(now())) |> pull(filename)))
```
...
```{r extract-pdf}
# Select the pdf that you want to import
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand Daily/2026/Strand_86(01.09.2026).pdf"

filename <- str_remove(path, "../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand Daily/2026/")
...
```{sql, connection = con}
SELECT
  site_id, sample_date, sample_time, censored_value, monitoring_group
FROM coastal.results
WHERE monitoring_group = 'daily' AND sample_date = '2026-09-01';
```
...
```{r extract-pdf}
# Select the pdf that you want to import
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/Atlantic Daily/Daily_84(31.08.2026).pdf"
...
```{sql, connection = con}
SELECT
  site_id, sample_date, sample_time, censored_value
FROM coastal.results
WHERE monitoring_group = 'daily' AND sample_date = '2026-08-31';
```
...
upload <- tibble(
  ...
  rename(censored_value = censored_value) |>
  mutate(censored_value = str_replace(censored_value, "None found", "ND")) |>
...
WITH cte AS
(SELECT
  site_id, sample_date, sample_time::time, analysis_completed, sample_temp_c, censored_value, filename_short AS filename, 'WLAB'::coastal.lab AS lab_code, 'daily'::coastal.monitoring_group AS monitoring_group
FROM saunders)
INSERT INTO coastal.results (site_id, sample_date, sample_time, analysis_completed, sample_temp_c, censored_value, filename, lab_code, monitoring_group)
SELECT * FROM cte;
```
...
(SELECT
  ...
  'WLAB'::coastal.lab AS lab_code
FROM coastal)
INSERT INTO coastal.high_frequency_samples (site_id, tag, sample_date, sample_time, analysis_completed, sample_temp_c, censored_value, filename, lab_code)
SELECT * FROM cte;
```

```{sql, connection = con}
SELECT
  site_id, tag, sample_date, sample_time, censored_value, sample_temp_c, filename
...
```{sql connection = con}
SELECT * FROM coastal.high_frequency_samples WHERE sample_date = '2025-05-27' AND censored_value = '20';
```
...
```{sql connection = con}
INSERT INTO coastal.triplicate_results (sample_id, site_id, sample_date, sample_time, analysis_completed, sample_temp_c, censored_value, filename, lab_code)
SELECT
  sample_id,
  site_id,
  sample_date,
...
```{sql, connection = con}
INSERT INTO coastal.triplicate_results (sample_id, site_id, sample_date, sample_time, sample_temp_c, censored_value, filename, lab_code)
SELECT
  sample_id,
  site_id,
  sample_date,
...
```{sql, connection = con}
SELECT filename, censored_value FROM coastal.triplicate_results WHERE sample_date > '2025-08-01'
...
SELECT
  ...
  censored_value
FROM lims)
INSERT INTO coastal.results (site_id, sample_date, censored_value, lab_code, monitoring_group)
SELECT
  site_id,
  sample_date,
  censored_value,
...
FROM blue_flag)
INSERT INTO coastal.results (site_id, sample_date, censored_value, filename, lab_code, monitoring_group)
SELECT * FROM cte;
```
...
sabs_daily_clean |>
  group_by(sample_date, site_id, censored_value) |>
  count() |>
  arrange(-n)
...
# Blue Flag Data Upload

## Extract data from scanned pdfs

```{r check-uploaded}
uploaded <- tbl(con, I("coastal.results")) |>
  filter(monitoring_group == "blue_flag") |>
  pull(filename)

sharepoint <- list.files("../../Infinity/Coastal Water Quality - Documents/Shared Results/Blue Flag/", pattern = "*.pdf")

to_upload <- setdiff(sharepoint, uploaded)
```

```{r exctract-scanned-data}
extracted <- tibble(
  filename_long = list.files("../../Infinity/Coastal Water Quality - Documents/Shared Results/Blue Flag/", pattern = "*.pdf", full.names = T),
  filename_short = list.files("../../Infinity/Coastal Water Quality - Documents/Shared Results/Blue Flag/", pattern = "*.pdf")
) |>
  filter(filename_short %in% to_upload) |>
  mutate(text = map(filename_long, ~pdf_ocr_text(.x, options = list(tessedit_pageseg_mode = 4), dpi = 300)))

blue_flag <- extracted |>
  unnest(text) |>
  mutate(text = str_split(text, "\n")) |>
  mutate(sample_date = map(text, ~keep(.x, ~str_detect(.x, "DATE SAMPLED")) |> str_extract("\\d{4}-\\d{2}-\\d{2}") |> ymd())) |>
  mutate(sample_time = map(text, ~keep(.x, ~str_detect(.x, "TIME SAMPLED"))|> str_extract("O{0,2}\\d{0,2}(h|H)\\d{0,2}O{0,2}") |> str_replace_all("O", "0") |> str_replace("h|H", ":"))) |>
  # mutate(sample_time = map(text, ~keep(.x, ~str_detect(.x, "TIME SAMPLED"))|> str_remove("TIME SAMPLED: ") |> str_trim() |> str_replace_all("O", "0") |> str_replace("h|H", ":"))) |>
  mutate(sample_temp_c = map(text, ~keep(.x, ~str_detect(.x, "Temperature")) |> str_extract("\\d{1,2}\\.\\d{1}") |> as.numeric())) |>
  mutate(site_name = map(text, ~keep(.x, ~str_detect(.x, "PRODUCT IDENTIFICATION")))) |>
  mutate(ecoli = map(text, ~keep(.x, ~str_detect(.x, "E.\\s?coli")))) |>
  mutate(enterococci = map(text, ~keep(.x, ~str_detect(.x, "Enterococci")))) |>
  select(filename_short, sample_date, sample_time, sample_temp_c, site_name, ecoli, enterococci) |>
  
  unnest(c(sample_date, sample_time, sample_temp_c, site_name, ecoli, enterococci), keep_empty = T) |>
  
  # Clean site names
  mutate(site_name = str_remove_all(site_name, "(PRODUCT IDENTIFICATION)|(Sea Water)|[:punct:]|[Ww]\\d{3}") |> str_trim()) |>
  mutate(ecoli = str_extract(ecoli, "(?<=(5|6)221)(.*)$") |> str_extract("(ND|>150|\\d{1,3})")) |>
  mutate(enterococci = str_extract(enterococci, "(?<=((7899-2)|(SANS me)))(.*)$") |> str_extract("(ND|>150|\\d{1,3})")) |>
  
  # Map to site IDs
   mutate(site_id = case_when(
    site_name |> str_to_lower() |> str_detect("bikini") ~ "XCS08",
    site_name |> str_to_lower() |> str_detect("camps bay") ~ "CN11",
    site_name |> str_to_lower() |> str_detect("clifton") ~ "CN09",
    site_name |> str_to_lower() |> str_detect("fish hoek") ~ "CS41",
    site_name |> str_to_lower() |> str_detect("hout bay") ~ "XCN10",
    site_name |> str_to_lower() |> str_detect("llandudno") ~ "XCN03",
    site_name |> str_to_lower() |> str_detect("melkbos") ~ "XCN07",
    site_name |> str_to_lower() |> str_detect("mnandi") ~ "CS19",
    site_name |> str_to_lower() |> str_detect("muizenberg") ~ "CS34",
    site_name |> str_to_lower() |> str_detect("seaforth") ~ "XCS13",
    site_name |> str_to_lower() |> str_detect("sil(w|v)erstroom") ~ "XCN14",
    site_name |> str_to_lower() |> str_detect("strandfontein") ~ "CS45",
    site_name |> str_to_lower() |> str_detect("strand beach") ~ "XCS26",
  )) |>
  select(site_id, site_name, sample_date, sample_time, sample_temp_c, ecoli, enterococci, filename_short) |>
  drop_na(site_id) |>
  
  # Pivot data
  pivot_longer(cols = c("ecoli", "enterococci"), names_to = "parameter", values_to = "censored_value") |>
  mutate(parameter = if_else(parameter == "ecoli", "Escherichia coli", "Enterococci"))
  
  # Check the data
  blue_flag |>
    copy_to(dest = con, name = "blue_flag", temporary = T, overwrite = T)
```
...
```{sql, connection = con}
SELECT * FROM coastal.results WHERE monitoring_group = 'blue_flag' ORDER BY sample_date DESC;
```

```{sql, connection = con}
INSERT INTO coastal.results (site_id, sample_date, sample_time, sample_temp_c, parameter, censored_value, filename, lab_code, monitoring_group)
SELECT
  site_id,
  sample_date::date,
  sample_time::time,
  ...
  parameter,
  censored_value,
  filename_short AS filename,
  'SABS' AS lab_code,
  'blue_flag' AS monitoring_group
FROM blue_flag;
```

```{sql, connection = con}
SELECT * FROM coastal.results WHERE monitoring_group = 'blue_flag' AND sample_date >= '2026-03-11';
```
...
```{r check-blue-flag-resamples}
coastal <- inner_join(
 tbl(con, I("coastal.sites_view")) |> collect(),
 tbl(con, I("coastal.results_view")) |> collect(),
 by = "site_id"
)
  
# Keep only the Blue Flag data
coastal |>
  filter(monitoring_group == "blue_flag", sample_date >= "2025-10-01") |>
  select(site_description, sample_date, censored_value) |>
  arrange(site_description, sample_date) |>
  distinct(sample_date) |>
  mutate(week = floor_date(sample_date, "week"), day = wday(sample_date, label = T, abbr = F)) |>
  complete(week, day) |>
  pivot_wider(names_from = day, values_from = sample_date)
```

# Upload ad-hoc samples

## Check which ad-hoc samples are not yet in the database

```{r check-adhoc}
results_adhoc <- tbl(con, I("coastal.results_adhoc")) |>
  select(filename) |>
  distinct() |>
  mutate(dataset = "adhoc") |>
  collect()

results <- tbl(con, I("coastal.results")) |>
  filter(monitoring_group == "adhoc") |>
  select(filename) |>
  distinct() |>
  mutate(dataset = "main") |>
  collect()

adhoc <- bind_rows(results_adhoc, results)

files_not_uploaded <- setdiff(list.files("../../Infinity/Coastal Water Quality - Documents/Shared Results/Ad-Hoc Samples/"), adhoc$filename)
```

```{r data-import}
tibble(
  filename_short = list.files("../../Infinity/Coastal Water Quality - Documents/Shared Results/Ad-Hoc Samples/", pattern = "*.pdf"),
  filename_long = list.files("../../Infinity/Coastal Water Quality - Documents/Shared Results/Ad-Hoc Samples/", pattern = "*.pdf", full.names = T)
) |>
  filter(filename_short %in% files_not_uploaded) |>
  mutate(pdf_data = map(filename_long, ~ pdf_text(.x) |>
    str_split("\n"))) |>
  unnest(pdf_data) |>
  mutate(sample_date = map(pdf_data, ~keep(.x, ~str_detect(.x, "Sample taken")) |> str_remove("\\*Sample taken:") |> str_trim() |> dmy())) |>
  ...
  mutate(data = map(pdf_data, ~{
    .x |>
      head_while(~!str_detect(.x, "colony forming units")) |>
      tail_while(~!str_detect(.x, "cfu per 100 m")) |>
      discard(~.x == "")
  })) |>
  # mutate(site_id = map(data, ~str_extract(.x, "[A-Z]{2,3}\\d{2}"))) |>
  # mutate(censored_value = map(data, ~str_extract(.x, "\\d{1,3}$"))) |>
  # mutate(sample_time = map(data, ~str_extract(.x, "\\d\\dh\\d\\d") |> str_replace("h", ":"))) |>
  mutate(site_id = "CN09") |>
  mutate(censored_value = "109") |>
  mutate(sample_time = "12:05") |>
  select(-pdf_data, -data) |>
  unnest() |>
  select(site_id, sample_date, sample_time, analysis_completed, sample_temp_c, censored_value, filename_short) |>
  unnest() |>
  
  # Upload to database
  copy_to(dest = con, name = "adhoc", temporary = T, overwrite = T)
```
...
```{sql, connection = con}
INSERT INTO coastal.results (site_id, sample_date, sample_time, analysis_completed, sample_temp_c, parameter, censored_value, unit, filename, lab_code, monitoring_group)
SELECT
  site_id,
  sample_date::date,
  sample_time::time,
  analysis_completed,
  sample_temp_c,
  'Enterococci' AS parameter,
  censored_value,
  'cfu per 100 mL' AS unit,
  filename_short AS filename,
  'WLAB' AS lab_code,
  'adhoc' AS monitoring_group
FROM adhoc;
```

```{sql, connection = con}
SELECT * FROM coastal.results WHERE sample_date = '2026-04-17' AND monitoring_group = 'adhoc';
```
...
# Authenticating with Supabase REST API

```{r creating-a-login-with-supabase}
# Set environment variables
Sys.setenv("API_KEY" = "sb_publishable_WdjSFiirJ1Hf5R46QXHgTQ_JTCXC9Wb",
           "BASE_URL" = "https://wnrvdvesovnkmqbhkhjz.supabase.co/",
           "EMAIL" = "francois.vanschalkwyk@outlook.com",
           "PASSWORD" = "TheBiggestZ49")

# Request User Sign Up
signup_request <- request("https://wnrvdvesovnkmqbhkhjz.supabase.co") |>
  req_url_path_append("auth/v1/signup") |>
  req_headers_redacted(
    "apikey" = Sys.getenv("SUPABASE_CLIENT_API_KEY")
  ) |>
  req_body_json(list(email = "francois.vanschalkwyk@outlook.com", password = "TheBiggestZ49"))

signup_response <- request2 |> req_perform()

# Log in as a user
login_request <- request(Sys.getenv("BASE_URL")) |>
  req_method("POST") |>
  req_url_path_append("auth/v1/token") |>
  req_url_query(grant_type = "password") |>
  req_headers(
...
successful <- login_response |>
  resp_body_json()

# Set user JWT as environmental variable
Sys.setenv("USER_JWT" = successful$access_token, "REFRESH_TOKEN" = successful$refresh_token)

data_request <- request(Sys.getenv("BASE_URL")) |>
  req_url_path_append("rest/v1/overview") |>
  req_headers_redacted(
    "apikey" = Sys.getenv("API_KEY"),
    "Accept-Profile" = "api"
  ) |>
  req_auth_bearer_token(Sys.getenv("USER_JWT"))

data_response <- data_request |> req_perform()
...
# Refresh Token
refresh_request <- request(Sys.getenv("BASE_URL")) |>
  req_method("POST") |>
  req_url_path_append("auth/v1/token") |>
  req_url_query(grant_type = "refresh_token") |>
  req_headers(
    "apikey" = Sys.getenv("API_KEY"),
    "Content-Type" = "application/json"
  ) |>
  req_body_json(
    list(refresh_token = Sys.getenv("REFRESH_TOKEN"))
  )

refresh_response <- refresh_request |> req_perform()

body <- refresh_response |>
  resp_body_json()

# Set user JWT as environmental variable
Sys.setenv("USER_JWT" = body$access_token, "REFRESH_TOKEN" = body$refresh_token)
```

# Upload Strand high-frequency sampling data

```{r}
#| label: upload-bacteria

# Get filenames
tibble(
  filename_long = list.files("../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand High-Frequency 2026/WALAB/", full.names = T),
  filename = list.files("../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand High-Frequency 2026/WALAB/")
) |>
  mutate(pdf_data = map(filename_long, ~ pdf_text(.x) |> str_split("\n"))) |>
  unnest(pdf_data) |>
  
  # Extract sample date
  mutate(sample_date = map(pdf_data, ~ {
    .x |>
      keep(~ str_detect(.x, "Sample taken")) |>
      str_remove("\\*Sample taken:\\s*") |>
      dmy()
  })) |>
  
  # Extract analysis completed date
  mutate(analysis_completed = map(pdf_data, ~ {
    .x |>
      keep(~ str_detect(.x, "Analysis completed:")) |>
      str_remove("Analysis completed:\\s*") |>
      dmy()
  })) |>
  
  # Extract sample temperature
  mutate(sample_temperature = map(pdf_data, ~ {
    .x |>
      keep(~ str_detect(.x, "Sample temperature:")) |>
      str_remove_all("Sample temperature:\\s*|ºC") |>
      as.numeric()
  })) |>
  ...
  mutate(
    site_id = map(data, ~ {
      str_extract_all(.x, "(XCS26)|(XCS34)")
    }),
    sample_time = map(data, ~{
      str_extract_all(.x, "\\d{2}h\\d{2}")
    }),
    tag = map(data, ~{
      str_extract_all(.x, "\\s[ABCD]\\s") |>
        str_trim()
    }),
    censored_value = map(data, ~{
      str_extract_all(.x, "\\s(\\d{1,4}|(None found))($|\\s)") |>
        str_trim() |>
        unlist()
    })
  ) |>
  pull(censored_value)
  unnest(sample_date, analysis_completed, sample_temperature, site_id, sample_time, tag, censored_value) |>
  select(site_id, sample_date, analysis_completed, sample_time, tag, sample_temperature, censored_value, filename) |>
  mutate(tag = replace_na(tag, "A")) |>
  arrange(filename, sample_date, site_id, sample_time)

# Upload to database
bacteria |>
  copy_to(dest = con, name = "bacteria", temporary = T, overwrite = T)
```
...
INSERT INTO princessvlei.water_results_microbiology (result_id, sample_date, analysis_completed, sample_temperature, parameter, unit, censored_value, filename)
SELECT
  b.result_id,
  c.sample_date,
  ...
  c.censored_value,
  c.filename
FROM princessvlei.water_samples a
  JOIN princessvlei.water_results b USING (sample_id)
  JOIN bacteria c ON a.label = c.sample_id AND b.suite = 'microbiology';
```
...
FROM princessvlei.water_samples a
  JOIN princessvlei.water_results b USING (sample_id)
  JOIN princessvlei.water_results_microbiology c USING (result_id)
WHERE a.monitoring_group = 'Pollution Tracking' AND b.suite = 'microbiology';
```
