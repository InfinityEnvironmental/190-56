...
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/SSB CMB Monitoring/CWQ_36(27.02.2026).pdf"
...
WHERE sample_date = '2026-02-27' AND monitoring_group = 'routine';
```
...
SELECT
  'XCS26' AS site_id,
  '2025-12-19'::date AS sample_date,
  '09:00'::time AS sample_time,
  '2025-12-21'::date AS analysis_completed,
   10.2 AS sample_temp_c,
  '26' AS enterococci_cfu_per_100ml,
  'Oelofse G.237.pdf' AS filename,
  'WLAB'::coastal.lab AS lab_code,
  'daily'::coastal.monitoring_group AS monitoring_group;
```

```{sql connection = con}
SELECT * FROM coastal.results WHERE sample_date = '2025-12-19' AND monitoring_group = 'daily';
```
...
```{r extract-pdf}
# Select the pdf that you want to import
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand Daily/Oelofse G.35.pdf"
...
FROM coastal.results
WHERE monitoring_group = 'daily' AND sample_date = '2026-02-27';
```
...
```{r extract-pdf}
# Select the pdf that you want to import
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/Atlantic Daily/Infinity Environmental AD.39.pdf"
...
blue_flag <- extracted |>
  ...
  mutate(enterococci = if_else(site_id == "CS45" & sample_date == "2026-02-17", "13", enterococci)) |>
  drop_na(site_id) |>
...
```{sql, connection = con}
SELECT * FROM coastal.results WHERE monitoring_group = 'blue_flag' AND sample_date > '2026-01-13';
```
...
tibble(
  ...
  mutate(site_id = "CN21") |>
  mutate(censored_value = "101") |>
  mutate(sample_time = "08:03") |>
...
```{sql, connection = con}
SELECT * FROM coastal.results WHERE sample_date = '2026-02-27' AND monitoring_group = 'adhoc';
```
