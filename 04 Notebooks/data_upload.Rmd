...
extracted <- tibble(
  ...
  mutate(text = map(filename_long, ~pdf_ocr_text(.x, options = list(tessedit_pageseg_mode = 4), dpi = 200))) |>
...
blue_flag <- extracted |>
  ...
  unnest(sample_date, sample_time, sample_temp_c, site_name, ecoli, enterococci) |>
  ...
  # Check the data
  blue_flag |>
    mutate(enterococci = if_else(site_id == "CS45" & sample_date == "2025-12-05", "31", enterococci)) |>
    mutate(sample_time = if_else(site_id == "XCN07" & sample_date == "2026-02-10", "09:05", sample_time)) |>
...
```{sql, connection = con}
INSERT INTO coastal.results (site_id, sample_date, sample_time, sample_temp_c, censored_value, filename, lab_code, monitoring_group, parameter)
SELECT
  site_id,
  sample_date::date,
  sample_time::time,
  ...
  'Escherichia coli' AS parameter
FROM blue_flag;
```

```{sql, connection = con}
SELECT * FROM coastal.results WHERE parameter = 'Escherichia coli';
```
