...
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/SSB CMB Monitoring/CWQ_39(04.03.2026).pdf"
...
WHERE sample_date = '2026-03-04' AND monitoring_group = 'routine';
```
...
```{r extract-pdf}
# Select the pdf that you want to import
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/Strand Daily/Oelofse G.38.pdf"
...
FROM coastal.results
WHERE monitoring_group = 'daily' AND sample_date = '2026-03-04';
```
...
```{r extract-pdf}
# Select the pdf that you want to import
path <- "../../Infinity/Coastal Water Quality - Documents/Shared Results/Atlantic Daily/Infinity Environmental AD.46.pdf"
...
tibble(
  ...
  mutate(site_id = "CN21") |>
  mutate(censored_value = map(data, ~str_extract(.x, "\\d{2,3}$"))) |>
  mutate(sample_time = map(data, ~str_extract(.x, "\\d{2}h\\d{2}") |> str_replace("h", ":"))) |>
