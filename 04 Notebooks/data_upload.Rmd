...
extracted |>
  ...
  mutate(sample_time = map(text, ~keep(.x, ~str_detect(.x, "TIME SAMPLED"))|> str_extract("O?\\d{1,2}(h|H)\\d{2}") |> str_replace("O", "0") |> str_replace("h|H", ":"))) |>
  ...
  # Clean site names
  mutate(site_name = str_remove_all(site_name, "(PRODUCT IDENTIFICATION)|(Sea Water)|[:punct:]|W\\d{3}") |> str_trim()) |>
