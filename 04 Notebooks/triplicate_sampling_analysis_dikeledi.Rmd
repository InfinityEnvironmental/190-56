...
summary_df <- trip_data %>%
  group_by(lab_code, sample_date) %>%
  summarise(
    mean_enterococci = mean(enterococci_cfu_per_100ml, na.rm = TRUE),
    sd = sd(enterococci_cfu_per_100ml, na.rm = TRUE),
    .groups = "drop"
  )

summary_df$mean_enterococci <- as.numeric(summary_df$mean_enterococci)
summary_df$sd <- as.numeric(summary_df$sd)
```
...
```{r plot-data}
ggplot(data = trip_data,
       mapping = aes(x=lab_code, y=enterococci_cfu_per_100ml, fill = lab_code, alpha = 1))+
  geom_boxplot()+
  facet_wrap(~site_id)+
...
ggplot(trip_sd, aes(x = lab_code, y = triplicate_sd, fill = lab_code, alpha = 10)) +
  geom_boxplot() +
  facet_wrap(~ site_id) +
...
ggplot(summary_df, aes(x = lab_code, y = mean_enterococci, fill = as.factor(sample_date))) +
  geom_col(position = position_dodge()) +
  geom_errorbar(aes(
    ymin = mean_enterococci - sd,
    ymax = mean_enterococci + sd),
                width = 0.2, color = "black") +
  labs(
    title = "Enterococci Concentration per Lab and Sampling Date",
    x = "Lab",
