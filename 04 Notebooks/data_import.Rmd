...
```{r read-blue-flag-data, fig.width = 7, fig.height = 10, dpi = 600}
...
blue_flag_append <- blue_flag_append |>
  mutate(site = "N")
...
blue_flag_data |>
  mutate(sample_date = date(sample_date)) |>
  group_by(sheet) |>
  summarise(min_date = min(sample_date), max_date = max(sample_date), n()) |>
  arrange(sheet)
  ggplot(aes(y = sheet, x = min_date, xend = max_date)) +
  geom_segment() +
  scale_x_date(minor_breaks = "1 year")

# Add sites codes to Blue Flag data
blue_flag_data <- blue_flag_data |>
  mutate(site = case_when(
    sheet |> str_to_lower() |> str_detect("bikini") ~ "XCS08",
    sheet |> str_to_lower() |> str_detect("camps bay") ~ "CN11",
    sheet |> str_to_lower() |> str_detect("clifton") ~ "CN09",
    sheet |> str_to_lower() |> str_detect("fish") ~ "XCS16",
    (sheet |> str_to_lower() |> str_detect("lland")) & str_detect(site, "Confl") ~ "XCN03A",
    (sheet |> str_to_lower() |> str_detect("lland")) & (!str_detect(site, "XCN03A") | is.na(site)) ~ "XCN03",
    str_detect(sheet, "Melkbos") & str_detect(sheet, "N") ~ "XCN07",
    str_detect(sheet, "Melkbos") & str_detect(sheet, "S") ~ "XCN15",
    str_detect(sheet, "Melkbosstrand") & str_detect(site, "S") ~ "XCN15",
    str_detect(sheet, "Melkbosstrand") & (!str_detect(site, "S") | is.na(site)) ~ "XCN07",
    sheet |> str_to_lower() |> str_detect("mnandi") ~ "CS19",
    sheet |> str_to_lower() |> str_detect("muiz") ~ "CS34",
    sheet |> str_to_lower() |> str_detect("seaforth") ~ "XCS13",
    sheet |> str_to_lower() |> str_detect("silwer") ~ "XCN14",
    sheet |> str_to_lower() |> str_detect("strandf") ~ "CS45",
    .default = site
  )) |>
  arrange(sheet)

# Check the site periods
blue_flag_data |>
  mutate(month = floor_date(date(sample_date), "month")) |>
  group_by(site, month) |>
  count() |>
...
# Check NA values
blue_flag_data |>
  filter(if_any(everything(), is.na)) # Decision to keep genuine missing values
...
blue_flag_data <- blue_flag_data |>
  select(site, sample_date, faecal_streptococci_cfu_per_100ml, file)

# Check the range of values for faecal
blue_flag_data |>
  mutate(faecal_streptococci_cfu_per_100ml = as.numeric(faecal_streptococci_cfu_per_100ml)) |>
  distinct(faecal_streptococci_cfu_per_100ml) |>
  arrange(faecal_streptococci_cfu_per_100ml)

# Create censored values
blue_flag_data <- blue_flag_data |>
  mutate(faecal_streptococci_cfu_per_100ml = case_when(
    ...
    .default = faecal_streptococci_cfu_per_100ml
  ))
...
blue_flag_data |>
  mutate(numeric_value = case_when(
    str_detect(faecal_streptococci_cfu_per_100ml, ">") ~ str_remove(faecal_streptococci_cfu_per_100ml, ">") |> as.numeric(),
    .default = faecal_streptococci_cfu_per_100ml |> as.numeric()
  )) |>
  ggplot(aes(x = sample_date, y = numeric_value, colour = site)) +
  geom_point()

# Change the names of the columns
blue_flag_data |>
  rename(site_id = site, censored_value = faecal_streptococci_cfu_per_100ml) |>

# Export the clean data
write_csv("02 Output Data/blue_flag_clean.csv")
```

The only site that has data for 2017/2018 summer is Seaforth.
...
```{r read-excel}
site_order <- read_excel("01 Input Data/CWQ sites numbered.xlsx",
           skip = 2,
           col_names = c("site_order", "site_name", "site_id", "sample_frequency")) |>
  filter(!if_any(everything(), is.na)) |>
  select(site_id, site_order)

copy_to(con, site_order, name = I("coastal.site_order"), temporary = FALSE)
```
