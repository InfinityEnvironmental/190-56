...
```{r run-function}
# Run the function with one file
read_bacteria_data("01 Input Data/Coastal Water Quality Data/60_Dalebrook_Tidal_Pool_(CS03).xlsm", 2)
...
ecoli |>
  ...
  geom_point() +
  theme(legend.position = "none") +
  coord_cartesian(xlim = c(ymd("2022-01-01"), ymd("2025-06-01")), ylim = c(0, 250))
```
...
faecal |>
  ...
  geom_point() +
  theme(legend.position = "none") +
  coord_cartesian(xlim = c(ymd("2023-01-01"), ymd("2025-06-01")), ylim = c(0, 1000))
```
...
ecoli <- ecoli |>
  ...
  group_by(site_id, date, full, value_cfu_per_100ml) |>
  slice_head(n = 1) |>
  ungroup()
...
faecal <- faecal |>
  ...
  group_by(site_id, date, value_cfu_per_100ml, full) |>
  slice_head(n = 1) |>
  ungroup()
```
...
```{r ssb-values}
ecoli <- ecoli |>
  group_by(date, site_id, value_cfu_per_100ml) |>
  slice_head(n = 1)
...
# Results table
ecoli <- ecoli |>
  select(date, site_id, value_cfu_per_100ml, censored_value)
```

## Check differences between CMB and SSB data
