...
for (site in unique_sites) {
  site_violin_data_monthly <- violin_data_monthly %>%
    filter(site_id == site)

  plot <- ggplot(site_violin_data_monthly, aes(x = month, y = numeric_value, fill = hazen_category)) +
    geom_violin(trim = TRUE) +
    ...
    scale_y_log10() + 
    labs(
      title = paste("Monthly Climatology of Enterococci distribution - Site:", site),
      x = "Month",
      y = "Enterococci (log10 scale)",
      fill = "Hazen Category"
    ) +
...
```{r Seasonality-across-every-yr, fig.height = 12}
...
if (num_years > 0) {
  ...
  years_group1 <- sort(unique_years)[1:mid_point+1]
  years_group2 <- sort(unique_years)[(mid_point + 2):num_years]
  ...
  plot_group1 <- violin_data_monthly2 %>%
    ...
    geom_point(aes(group = month),
               ...
               size = 0.8) +
    scale_y_log10() +
    facet_wrap(~ year, ncol = 3, strip.position = "right") + 
    labs(
