...
# Clean the enterococci_cfu_per_100ml column
#results_cleaned <- results_sorted %>%
 # mutate(
  #  enterococci_cfu_per_100ml_trimmed = trimws(enterococci_cfu_per_100ml),
   # enterococci_numeric = round(case_when(
    #  grepl("^ND$", enterococci_cfu_per_100ml_trimmed, ignore.case = TRUE) ~ 0,
     # grepl("^>", enterococci_cfu_per_100ml_trimmed) ~ as.numeric(gsub(">", "", enterococci_cfu_per_100ml_trimmed)) * 3,
      #grepl("^<", enterococci_cfu_per_100ml_trimmed) ~ as.numeric(gsub("<", "", enterococci_cfu_per_100ml_trimmed)) / 3,
      #grepl(" ", enterococci_cfu_per_100ml_trimmed) ~ as.numeric(gsub(" ", "", enterococci_cfu_per_100ml_trimmed)),
      #TRUE ~ as.numeric(enterococci_cfu_per_100ml_trimmed)
  #  ), digits = 0) # Round to 0 decimal places
#  ) %>%
  #select(-enterococci_cfu_per_100ml_trimmed)
```
...
```{R Plotting monthly/yearly 90th & 95th Hazen percentiles of routine monitoring group}
...
```{R Calculate monthly/daily 90th & 95th Hazen percentiles for daily monitoring group}
...
```{r Plotting monthly/daily 90th & 95th Hazen percentiles for daily monitoring group}
...
#DAILY 
#(90th & 95th Percentiles =; only 1 unique data point for each site, for each lab, for each day)
...
```{R Calculating Blue flag monthly/yearly 90th & 95th Hazen percentiles}
...
#90th & 95th Hazen percentiles =; every month of blue flag data has only one data point for each month, for each site and each lab
...
```{R Plotting Blue flag monthly/yearly 90th & 95th Hazen percentiles}
