...
sites_bbox <- st_bbox(obj = c(xmin = 18.25, xmax = 18.9, ymin = -34.40, ymax = -33.6))
```
...
```{r open-street-map-data, fig.width = 8.3, fig.height = 11.7, dpi = 600}
# Access coastline data
coastline <- opq(sites_bbox) |>
  add_osm_feature("natural", "coastline") |>
  osmdata_sf()

# Crop to City of Cape Town
coastline$osm_lines |>
  ggplot() +
  geom_sf() +
...
# Create a bounding box for one site (Camps Bay South)
bbox <- sites_sf |>
  filter(site_id == "CN11") |>
...
# Plot the coastline
ggplot() +
  geom_sf(data = coastline$osm_lines |> st_transform("ESRI: 102562")) +
  geom_sf(data = sites_sf |> st_transform("ESRI: 102562")) +
  coord_sf(xlim = c(bbox[1], bbox[3]), ylim = c(bbox[2], bbox[4]))
```

```{r access-beach-data, fig.width = 8.3, fig.height = 11.7, dpi = 600}
# Make the coastline a polygon
coastline$osm_lines |>
  ggplot() +
  geom_sf()
```
...
```{r access-beach-data}
# Access beach data
beaches <- opq(sites_bbox) |>
  add_osm_feature("natural", "beach") |>
  osmdata_sf()

# Plot the beach data for a specific site
ggplot() +
  geom_sf(data = beaches$osm_polygons |> st_transform("ESRI: 102562")) +
  geom_sf(data = coastline$osm_lines |> st_transform("ESRI: 102562")) +
  geom_sf(data = coastline$osm_polygons |> st_transform("ESRI: 102562")) +
  geom_sf(data = sites_sf |> st_transform("ESRI: 102562")) +
  coord_sf(xlim = c(bbox[1], bbox[3]), ylim = c(bbox[2], bbox[4]))
```
...
streets <- opq(sites_bbox) |>
  add_osm_feature("highway", c("motorway", "motorway_link", "primary", "primary_link", "secondary", "secondary_link")) |>
  osmdata_sf()
streets$osm_lines

# Plot all data for a specific site
ggplot() +
  geom_sf(data = beaches$osm_polygons |> st_transform("ESRI: 102562"), fill = "grey", colour = NA, alpha = 0.5) +
  geom_sf(data = coastline$osm_lines |> st_transform("ESRI: 102562"), colour = "white") +
  geom_sf(data = coastline$osm_polygons |> st_transform("ESRI: 102562"), colour = NA, fill = "grey", alpha = 0.5) +
  geom_sf(data = streets$osm_lines |> st_transform("ESRI: 102562"), colour = "white") +
...
tibble(
  ...
  scale_fill_brewer(type = "seq", palette = ) +
