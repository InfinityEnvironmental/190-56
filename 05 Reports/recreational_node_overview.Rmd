...
ggplot() +
  geom_sf(data = cct, colour = NA) +
  geom_sf(data = streets$osm_lines, colour = "white")
...
```{r build-basemap, fig.height = 6, fig.width=6, dpi = 1000}
...
# Crop the streets data to the circle
cropped_streets <- st_intersection(streets$osm_lines, circle)
...
sites |>
  ...
  ggplot() +
  geom_rect(aes(xmin = 0.950, xmax = 1.050, ymin = 0.99, ymax = 1.01, fill = hazen_category)) +
  geom_text(aes(x = 1, y = 1, label = hazen_category), family = "Century Gothic", size = 50, hjust = 0.5, colour = "white") +
  theme(
