...
circle <- sites |>
  filter(site_id == site) |>
  st_buffer(1500, nQuadSegs = 10000)
...
site_map <- ggplot() +
  ...
  geom_sf(data = cropped_streets$osm_lines, colour = "white") +
  geom_sf(data = cropped_water$osm_polygons, colour = "lightblue") +
  geom_sf(data = circle, colour = city_pink, fill = NA, size = 5) +
