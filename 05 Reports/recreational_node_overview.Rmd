...
site_overview <- ggplot() +
  ...
  geom_sf(data = sites_sf |> filter(site_id == site_code), colour = city_pink, size = 7, alpha = 0.7, shape = 16) +
  ...
  scale_size_manual(values = c("Recreational Node" = 4, "Coastal Monitoring Point" = 2))
```
...
(site_map <- ggplot() +
  ...
  # Add the current monitoring poin3
...
hazen_categories <- bind_rows(all, summer, winter) |>
  ...
  geom_text(aes(x = hazen_category, y = "Season", label = hazen_category), family = "Century Gothic", colour = "white", size = 3.5) +
...
# Style components
  theme(
    ...
    legend.position = "none",
    strip.background = element_blank(),
    strip.text = element_text(colour = "white", family = "Century Gothic", size = 10, face = "bold")
  )
```
...
(infographic <- base +
  ...
  inset_element(results_plot, left = unit(23, "mm"), bottom = unit(47, "mm"), right = unit(105, "mm"), top = unit(150, "mm"), align_to = "full") +
  inset_element(hazen_categories, left = unit(15, "mm"), bottom = unit(147, "mm"), right = unit(136.5, "mm"), top = unit(216.9, "mm"), align_to = "full") +
...
```{r save-figure}
ggsave(str_c(site_code, ".jpg", sep = ""), plot = infographic, path = "../../Infinity/Coastal Water Quality - Documents/KYC 2025/Draft Recreational Node Overviews/", width = 210, height = 223.8375, units = "mm", dpi = 600)
