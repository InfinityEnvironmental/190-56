...
# Plot the sites
site_map <- ggplot() +
  geom_sf(data = cct_cropped, colour = NA) +
  geom_sf(data = sites_sf_cropped, aes(colour = category, size = category), shape = 16) +
  geom_sf(data = sites_sf_cropped |> filter(site_id == "CN09"), colour = city_pink, size = 3) +
  geom_rect(aes(xmin = bbox[1], ymin = bbox[2], xmax = bbox[3], ymax = bbox[4]), fill = NA, colour = "darkgrey") +
  geom_sf_text(data = sites_sf_cropped, aes(label = site_description), colour = "white", family = "Century Gothic", hjust = 1, vjust = -1.5, nudge_x = -100, check_overlap = TRUE, size = 2) +
  scale_colour_manual(values = c("Recreational Node" = city_green, "Coastal Monitoring Point" = "darkgrey")) +
