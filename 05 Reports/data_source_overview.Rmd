...
```{r plot-all-data}
sample_counts <- tbl(con, I("coastal.results_view")) |>
  collect() |>
  mutate(year = year(sample_date), month = month(sample_date, label = TRUE)) |>
  group_by(year, month, monitoring_group) |>
  count()
...
sample_counts_plot <- sample_counts |>
  ...
  ggplot(aes(x = month, y = n, fill = monitoring_group)) +
  labs(title = "Sample Frequency for Coastal Water Quality",
       y = "Number of samples",
       fill = "Monitoring Group",
       subtitle = "City of Cape Town") +
  geom_col() +
  facet_wrap(facets = vars(year), nrow = 3) +
  scale_fill_manual(values = c("Blue Flag" = city_blue, "Routine" = city_green, "Daily" = city_pink)) +
  theme(
    text = element_text(family = "Century Gothic"),
    legend.position = "bottom",
...
ggsave("sample_frequency.jpg", path = "../02 Output Data/", plot = sample_counts_plot, width = 8, height = 4, dpi = 600)
```
